...
class RegexList {
  constructor() {
    ...
    // Initialize with default regexes
    this.quoteHeadersRegex = this.buildRe2(this.defaultQuoteHeadersRegex);
    this.signatureRegex = this.buildRe2(this.defaultSignatureRegex);
    
    // Keep backward compatibility with answerHeadersRegex
    this.answerHeadersRegex = this.quoteHeadersRegex;
  }
  ...
  buildSafeRegExp(regex) {
    return new this.SafeRegExp(regex);
  }
